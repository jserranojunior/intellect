...
import { dateUStoJsFull } from "alvitre-obelisk/src/convertDates/js/convertDateToJs";
...
class Financial{
  ...
  async getCategoriaContas(
    ...
    return await FuncHttpFinanceiro()
      .get(url)
      .then((res: any) => {
        console.info("getCategoriaContas",res,selectedDate)
        return res;
      }).catch((err:any) =>  {
        console.error(err)
      })
  ...
  async setValuesFormBillsToPay() {
    ...
    this.store.inicioDataPagamentoWithouFormated = dateJstoUs(setDate);
    console.log("this.store.inicioDataPagamentoWithouFormated", this.store.inicioDataPagamentoWithouFormated)
  }
  ...
  async getSetContasAPagar(id: number) {
    await this.GetEditBillsToPay(id).then((res) => {
      if(res.data[0].inicio_data_pagamento){
        this.store.inicioDataPagamentoWithouFormated = dateJstoUs(res.data[0].inicio_data_pagamento)
      }
      if(res.data[0].fim_data_pagamento){
        this.store.fimDataPagamentoWithouFormated = dateJstoUs(res.data[0].fim_data_pagamento)
